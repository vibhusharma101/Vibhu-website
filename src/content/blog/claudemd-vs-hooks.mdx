...
Here's the three-layer model I now use:

| Layer | Tool | What It Does | Enforced? |
|-------|------|-------------|-----------|
| 1 | `CLAUDE.md` | Tells the AI about your project and preferences | No — advisory |
| 2 | Hooks | Gates that block violations before they happen | Yes — automatic |
| 3 | Tests & CI | Verifies the final output is correct | Yes — blocking |

Use `CLAUDE.md` for *orientation* — your tech stack, your style guide, your preferences. Claude genuinely uses this.

Use hooks for *constraints* — rules that cannot be violated, ever, full stop.

Use tests for *verification* — proof that the output is actually correct.
...
If you want to start experimenting with hooks, here are three steps that take ten minutes:

**Step 1: Add visibility first (no blocking)**
...
```json
{
  "hooks": {
    "PreToolUse": [{
      "matcher": "Write",
      "hooks": [{ "type": "command", "command": "echo '[HOOK] About to write a file'" }]
    }]
  }
}
```

This just prints a message when Claude is about to write. No blocking. Pure observation — you see exactly when and how often Claude uses its tools.

**Step 2: Log what Claude is writing**

```bash
# .claude/hooks/log-writes.sh
#!/bin/bash
echo "[$(date)] Claude writing file" >> .claude/activity.log
cat - >> .claude/activity.log
exit 0
```

This captures every file Claude writes to a local log. Start here before you start blocking — understand the traffic before you put up gates.

**Step 3: Add your first real enforcement**

Once you know what patterns to look for, write a script that reads stdin (the content Claude is about to write), checks for violations, and exits `1` with a helpful error message if it finds one.

The error message matters — Claude reads it and uses it to correct course. Be specific: "found custom Button component, use `<Button>` from `@mantine/core` instead" is infinitely more useful than "rule violation."
