...
Behind that button is a job that talks to a third-party ERP, pulls a few thousand records, transforms them, and writes them back. It takes anywhere from 8 seconds to 4 minutes. And for a while, we ran it right there on the request thread.

Then one afternoon I watched a sync die the instant my dev server hot-reloaded. The row sat in the database, stuck in `RUNNING`, forever. Nobody was coming back to finish it.
...
The instinct is "add a queue." But we didn't have a backend consumer service, the work needed the full app context (dependency injection, models, config), and standing up SQS + a new consumer for one feature felt like bringing a crane to hang a picture frame.

So we did the boring, durable thing: **put the queue in the database we already have.**
