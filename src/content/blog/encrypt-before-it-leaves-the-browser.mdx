...
TLS is one layer. For a credential that unlocks someone else's system, one layer isn't enough. Here's the defense-in-depth version we built.
...
```js
const plaintext = privateDecrypt(rsaPrivateKey, ciphertext);
...
One RSA keypair forever is a liability. We use a small **keypair registry** so keys can rotate:
...
For a low-stakes internal tool? Maybe. For **credentials that unlock a customer's ERP, payment processor, or cloud account?** No. This is proportionate.
