...
Two things bothered me going in: **why does the standard midpoint formula have a production bug**, and **how do you actually prove $O(\log n)$** — not just assert it? Here's everything I worked through, with interactive demos so the intuition sticks.
...
**Setup:**
- Let $n$ = number of elements in the sorted array
- Let $k$ = number of iterations in the worst case
...
Use the logarithmic power rule — $\log(a^b) = b \cdot \log(a)$:
...
The maximum steps to search $n$ elements is $\log_2(n)$. Drop the base for Big-O notation: **$O(\log n)$**.
...
**2. The overflow bug isn't trivia.** The naive formula $\frac{low + high}{2}$ fails in production on large arrays. The safe formula $low + \frac{high - low}{2}$ is what you should actually write — and now you can prove why it's equivalent.

**3. Logarithms grow impossibly slowly.** $\log_2(10^9) \approx 30$. A problem $1000\times$ larger takes only $\sim\!10$ more steps. That asymptote is the whole game in algorithm design.
